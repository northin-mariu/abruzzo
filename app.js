(function () {
      ...
      if (o.plan && typeof o.plan === 'object') {
        Object.keys(o.plan).forEach(function (d) {
          ...
          SLOTS.forEach(function (s) {
            ...
            else if (v && typeof v === 'object' && typeof v.id === 'string' && (byId[v.id] || isNote(v.id))) {
      ...
      if (Array.isArray(o.ops)) {
        S.ops = o.ops.filter(function (x) {
          return x && typeof x === 'object' && typeof x.day === 'number' && typeof x.slot === 'string' &&
                 typeof x.at === 'number' && (x.e === null || (x.e && typeof x.e.id === 'string' && (byId[x.e.id] || isNote(x.e.id))));
        }).slice(-40);
      }
  ...
  // A typed note ("Pasta night", "leave by 9") rides in the id itself as x-pasta-night:
  // the worker's plan filter already accepts it, and old builds shelve unknown ids verbatim
  // (foreignPlan) instead of deleting them. Display re-spaces the slug; punctuation is lost,
  // which is the price of needing no server change.
  function noteId(t) {
    var s = String(t || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 30).replace(/-+$/, '');
    return s ? 'x-' + s : '';
  }
  function isNote(id) { return typeof id === 'string' && id.indexOf('x-') === 0 && !byId[id]; }
  ...
  function encodePlan() {
    ...
    DAYS.forEach(function (d) {
      ...
      SLOTS.forEach(function (s) {
        var e = slots[s.k];
        if (e && (byId[e.id] || isNote(e.id))) out.push('d' + d.d + '--' + s.k + '--' + e.id + '--' + slugName(e.by) + '--b' + (e.booked ? '1' : '0'));
      });
    });
  ...
  function decodePlan(list) {
    ...
    (list || []).forEach(function (s) {
      ...
      if (!m) return;
      if (!byId[m[3]] && !isNote(m[3])) { foreignPlan.push(s); return; }
  ...
  function replayOps(plan) {
    ...
    liveOps().forEach(function (o) {
      ...
      if (o.e && !byId[o.e.id] && !isNote(o.e.id)) return;
  ...
  function renderCalendar() {
    ...
    DAYS.forEach(function (day) {
      ...
      SLOTS.forEach(function (sl) {
        ...
        if (filled) {
          var p = byId[filled.id], note = !p && isNote(filled.id), g = !!(p && p.generic);
          var nm = note ? noteLabel(filled.id) : (p ? p.name : filled.id);
          var who = filled.by ? '<span class="sby" style="--who:' + whoColour(filled.by) + '">' + esc(filled.by) + '</span>' : '';
          var where = note ? who
                    : g ? (p.tbc ? esc(p.tbc) + (who ? ' · ' + who : '') : who)
                    : (p.mins === 0 ? 'at the house' : esc(p.town) + ' · ' + p.mins + ' min') + (who ? ' · ' + who : '');
          body += '<div class="slotrow' + (filled.booked ? ' bkd' : '') + '"><span class="splace">' + esc(nm) +
  ...
  function showPicker(btn, day, slot, mode) {
    ...
    input.type = 'search';
    input.placeholder = confirming ? 'Search for the place' : 'Search places \u2014 or type your own';
    ...
    function fill() {
      ...
      list.textContent = '';
      var nid = noteId(input.value);
      if (!pool.length) {
        msg.textContent = nid ? 'Nothing on the list matches \u2014 put it in below, in your own words'
      ...
      if (nid) {
        ...
        nb.className = 'pick pick-note';
        nb.type = 'button';
        nb.innerHTML = '<span>\u270F\uFE0F Put \u201C' + esc(input.value.trim()) + '\u201D straight in</span>' +
                       '<span class="t">your own words \u2014 not from the list</span>';
        nb.addEventListener('click', function () { choose({ id: nid }); });
        list.appendChild(nb);
      }
    }
